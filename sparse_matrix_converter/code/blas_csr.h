...
u32 CSR_SparseMatrixVectorMultiplication(csr *CSR, vector *In, vector *Out) {
  ...
  for (u32 I = 0; I < CSR->Header.NumberOfRows; ++I) {
    ...
    for (u32 J = RowStart; J < RowEnd; ++J) {
      ...
      ((f32 *)Out->Elements)[I] +=
          ((f32 *)CSR->Elements)[J] * ((f32 *)In->Elements)[ColumnIndex];
    }
  }
...
u32 CSR_SpMV(csr *CSR, vector *In, vector *Out) {
