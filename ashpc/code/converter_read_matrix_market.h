...
internal inline u32 MM_ReadLine(FILE *File, char *Line, u32 Length) {
...
internal inline void MM_PrintHeader(matrix_market_header *Header) {
  printf("%s %s %s %s %s\n", Header->BannerString, Header->ObjectString,
         Header->FormatString, Header->FieldString, Header->SymmetryString);
}

internal inline u32 MM_ReadFile(FILE *File, matrix_market *MatrixMarket) {
  u32 Error;
  char Line[LINE_LENGTH];
  ...
  for (u32 I = 0; I < MatrixMarket->Header.NonZeroElements; ++I) {
    ...
    if (MatrixMarket->Header.Field == REAL) {
      if (sscanf(Line, "%u %u %f", &MatrixMarket->Rows[I],
      ...
      if (sscanf(Line, "%u %u %lf", &MatrixMarket->Rows[I],
                 &MatrixMarket->Columns[I],
      ...
      if (sscanf(Line, "%u %u %d", &MatrixMarket->Rows[I],
                 &MatrixMarket->Columns[I],
...
internal inline u32 MM_ReadFileTyped(FILE *File, matrix_market *MatrixMarket) {
  u32 Error;
  char Line[LINE_LENGTH];
